...
//move elements from one stack to another for part1
...
void moveBatchStack(int times,int src,int dest,vector<stack<char>> &v){
    deque<char> dq;
    for(int i=0;i<times;i++){
        dq.push_front(v.at(src-1).top());
        v.at(src-1).pop();
    }
    while(!dq.empty()){
        v.at(dest-1).push(dq.front());
        dq.pop_front();
    }
